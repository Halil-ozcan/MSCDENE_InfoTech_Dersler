...
public class AuthService : IAuthService
{
    ...
    public AuthService(UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, IOptions<JwtConfig> optionsJwtConfig,ICartService cartService)
    {
    ...
    public async Task<ResponseDTO<AppUserDTO>> RegisterAsync(RegisterDTO registerDTO)
    {
        try
        {
            ...
            var cartCreateResult = await _cartService.CreateCartAsync(new CartCreateDTO{AppUserId=appUser.Id}); // sepet oluşturma
            if(!cartCreateResult.IsSuccessful)
            {
                return ResponseDTO<AppUserDTO>.Fail(cartCreateResult.Message, cartCreateResult.StatusCode);
            }
